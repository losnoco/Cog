...
{
#if defined(COUNT_INSTR)
   inc_m32rel(state, &state->instr_count[109]);
#endif
    gencallinterp(state, (unsigned long long)state->current_instruction_table.MFC0, 0);
}

void genmtc0(usf_state_t * state)
{
#if defined(COUNT_INSTR)
   inc_m32rel(state, &state->instr_count[110]);
#endif
