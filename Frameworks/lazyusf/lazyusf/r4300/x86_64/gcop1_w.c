...
void gencvt_d_w(usf_state_t * state)
{
   ...
   gencallinterp(state, (unsigned long long)state->current_instruction_table.CVT_D_W, 0);
#else
   gencheck_cop1_unusable(state);
   mov_xreg64_m64rel(state, RAX, (unsigned long long *)(&state->reg_cop1_simple[state->dst->f.cf.fs]));
   fild_preg64_dword(state, RAX);
