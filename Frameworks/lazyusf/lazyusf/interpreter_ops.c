...
void r4300i_J (usf_state_t * state) {
	...
	state->JumpToLocation = (state->PROGRAM_COUNTER & 0xF0000000) + (state->Opcode.target << 2);
...
void r4300i_BEQ (usf_state_t * state) {
	state->NextInstruction = DELAY_SLOT;
	if (state->GPR[state->Opcode.rs].DW == state->GPR[state->Opcode.rt].DW) {
		state->JumpToLocation = state->PROGRAM_COUNTER + ((int16_t)state->Opcode.offset << 2) + 4;
		TestInterpreterJump(state,state->PROGRAM_COUNTER,state->JumpToLocation,state->Opcode.rs,state->Opcode.rt);
	} else {
...
void r4300i_BLEZ (usf_state_t * state) {
	state->NextInstruction = DELAY_SLOT;
	if (state->GPR[state->Opcode.rs].DW <= 0) {
		state->JumpToLocation = state->PROGRAM_COUNTER + ((int16_t)state->Opcode.offset << 2) + 4;
		TestInterpreterJump(state,state->PROGRAM_COUNTER,state->JumpToLocation,state->Opcode.rs,0);
	} else {
		state->JumpToLocation = state->PROGRAM_COUNTER + 8;
	}
}
...
void r4300i_ADDI (usf_state_t * state) {
	if (state->Opcode.rt == 0) { return; }
	state->GPR[state->Opcode.rt].DW = (state->GPR[state->Opcode.rs].W[0] + ((int16_t)state->Opcode.immediate));
}
...
void r4300i_SLTI (usf_state_t * state) {
	if (state->GPR[state->Opcode.rs].DW < (int64_t)((int16_t)state->Opcode.immediate)) {
		state->GPR[state->Opcode.rt].DW = 1;
	} else {
		state->GPR[state->Opcode.rt].DW = 0;
	}
}

void r4300i_SLTIU (usf_state_t * state) {
	int32_t imm32 = (int16_t)state->Opcode.immediate;
	int64_t imm64;

	imm64 = imm32;
	state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rs].UDW < (uint64_t)imm64?1:0;
}

void r4300i_ANDI (usf_state_t * state) {
	state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rs].DW & state->Opcode.immediate;
}
...
void r4300i_LUI (usf_state_t * state) {
	if (state->Opcode.rt == 0) { return; }
	state->GPR[state->Opcode.rt].DW = (int32_t)((int16_t)state->Opcode.offset << 16);
}
...
void r4300i_DADDIU (usf_state_t * state) {
	state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rs].DW + (int64_t)((int16_t)state->Opcode.immediate);
}
...
void r4300i_LDL (usf_state_t * state) {
	...
	Address = state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset;
	Offset  = Address & 7;

	if (!r4300i_LD_VAddr(state,(Address & ~7),&Value)) {
		return;
	}
	state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rt].DW & LDL_MASK[Offset];
	state->GPR[state->Opcode.rt].DW += Value << LDL_SHIFT[Offset];
}
...
void r4300i_LDR (usf_state_t * state) {
	...
	state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rt].DW & LDR_MASK[Offset];
	state->GPR[state->Opcode.rt].DW += Value >> LDR_SHIFT[Offset];

}

void r4300i_LB (usf_state_t * state) {
	uint32_t Address =  state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset;
	if (state->Opcode.rt == 0) { return; }
	if (!r4300i_LB_VAddr(state,Address,&state->GPR[state->Opcode.rt].UB[0])) {
		TLB_READ_EXCEPTION(Address);
	} else {
		state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rt].B[0];
	}
}
...
void r4300i_LH (usf_state_t * state) {
	...
	if (!r4300i_LH_VAddr(state,Address,&state->GPR[state->Opcode.rt].UHW[0])) {
		//if (ShowTLBMisses) {
			DisplayError("LH TLB: %X",Address);
		//}
		TLB_READ_EXCEPTION(Address);
	} else {
		state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rt].HW[0];
	}
}
...
void r4300i_LWL (usf_state_t * state) {
	...
	state->GPR[state->Opcode.rt].DW = (int32_t)(state->GPR[state->Opcode.rt].W[0] & LWL_MASK[Offset]);
	state->GPR[state->Opcode.rt].DW += (int32_t)(Value << LWL_SHIFT[Offset]);
}
...
//	if ((Address & 3) != 0) { ADDRESS_ERROR_EXCEPTION(Address,1); }
	...
	if (!r4300i_LW_VAddr(state,Address,&state->GPR[state->Opcode.rt].UW[0])) {
		//if (ShowTLBMisses) {
			//printf("LW TLB: %X",Address);
		//}
		TLB_READ_EXCEPTION(Address);
	} else {
		state->GPR[state->Opcode.rt].DW = state->GPR[state->Opcode.rt].W[0];
	}
}
...
void r4300i_LBU (usf_state_t * state) {
	...
	if (!r4300i_LB_VAddr(state,Address,&state->GPR[state->Opcode.rt].UB[0])) {
		...
		state->GPR[state->Opcode.rt].UDW = state->GPR[state->Opcode.rt].UB[0];
	}
}
...
void r4300i_LHU (usf_state_t * state) {
	...
	if (!r4300i_LH_VAddr(state,Address,&state->GPR[state->Opcode.rt].UHW[0])) {
		...
		state->GPR[state->Opcode.rt].UDW = state->GPR[state->Opcode.rt].UHW[0];
	}
}
...
void r4300i_LWR (usf_state_t * state) {
	...
	state->GPR[state->Opcode.rt].DW = (int32_t)(state->GPR[state->Opcode.rt].W[0] & LWR_MASK[Offset]);
	state->GPR[state->Opcode.rt].DW += (int32_t)(Value >> LWR_SHIFT[Offset]);
}
...
void r4300i_LWU (usf_state_t * state) {
	...
	if (!r4300i_LW_VAddr(state,Address,&state->GPR[state->Opcode.rt].UW[0])) {
		...
		state->GPR[state->Opcode.rt].UDW = state->GPR[state->Opcode.rt].UW[0];
	}
}

void r4300i_SB (usf_state_t * state) {
	uint32_t Address =  state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset;
	if (!r4300i_SB_VAddr(state,Address,state->GPR[state->Opcode.rt].UB[0])) {
	}
}
...
void r4300i_SH (usf_state_t * state) {
	...
	if (!r4300i_SH_VAddr(state,Address,state->GPR[state->Opcode.rt].UHW[0])) {
	}
}
...
void r4300i_SWL (usf_state_t * state) {
	...
	Value += state->GPR[state->Opcode.rt].UW[0] >> SWL_SHIFT[Offset];
...
void r4300i_SW (usf_state_t * state) {
	...
	if (!r4300i_SW_VAddr(state,Address,state->GPR[state->Opcode.rt].UW[0])) {
	}
	//TranslateVaddr(&Address);
	//if (Address == 0x00090AA0) {
	//	LogMessage("%X: Write %X to %X",state->PROGRAM_COUNTER,state->GPR[state->Opcode.rt].UW[0],state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset);
	//}
}
...
void r4300i_SDL (usf_state_t * state) {
	...
	Value += state->GPR[state->Opcode.rt].UDW >> SDL_SHIFT[Offset];
...
void r4300i_SDR (usf_state_t * state) {
	...
	Value += state->GPR[state->Opcode.rt].UDW << SDR_SHIFT[Offset];
...
void r4300i_SWR (usf_state_t * state) {
	...
	Value += state->GPR[state->Opcode.rt].UW[0] << SWR_SHIFT[Offset];
...
void r4300i_LWC1 (usf_state_t * state) {
	uint32_t Address =  state->GPR[state->Opcode.base].UW[0] + (uint32_t)((int16_t)state->Opcode.offset);
	TEST_COP1_USABLE_EXCEPTION
	if ((Address & 3) != 0) { ADDRESS_ERROR_EXCEPTION(Address,1); }
	if (!r4300i_LW_VAddr(state,Address,&*(uint32_t *)state->FPRFloatLocation[state->Opcode.ft])) {
		//if (ShowTLBMisses) {
...
void r4300i_SC (usf_state_t * state) {
	...
	state->GPR[state->Opcode.rt].UW[0] = state->LLBit;
}

void r4300i_LD (usf_state_t * state) {
	uint32_t Address =  state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset;
	if ((Address & 7) != 0) { ADDRESS_ERROR_EXCEPTION(Address,1); }
	if (!r4300i_LD_VAddr(state,Address,&state->GPR[state->Opcode.rt].UDW)) {
	}
}
...
void r4300i_LDC1 (usf_state_t * state) {
	...
	TEST_COP1_USABLE_EXCEPTION
	if ((Address & 7) != 0) { ADDRESS_ERROR_EXCEPTION(Address,1); }
	if (!r4300i_LD_VAddr(state,Address,&*(uint64_t *)state->FPRDoubleLocation[state->Opcode.ft])) {
	}
}
...
void r4300i_SWC1 (usf_state_t * state) {
	...
	if (!r4300i_SW_VAddr(state,Address,*(uint32_t *)state->FPRFloatLocation[state->Opcode.ft])) {
	}
}
...
void r4300i_SDC1 (usf_state_t * state) {
	...
	if (!r4300i_SD_VAddr(state,Address,*(int64_t *)state->FPRDoubleLocation[state->Opcode.ft])) {
	}
}
...
void r4300i_SD (usf_state_t * state) {
	uint32_t Address =  state->GPR[state->Opcode.base].UW[0] + (int16_t)state->Opcode.offset;
	if ((Address & 7) != 0) { ADDRESS_ERROR_EXCEPTION(Address,0); }
	if (!r4300i_SD_VAddr(state,Address,state->GPR[state->Opcode.rt].UDW)) {
	}
}
/********************** R4300i state->Opcodes: Special **********************/
void r4300i_SPECIAL_SLL (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = (state->GPR[state->Opcode.rt].W[0] << state->Opcode.sa);
}

void r4300i_SPECIAL_SRL (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = (int32_t)(state->GPR[state->Opcode.rt].UW[0] >> state->Opcode.sa);
}
...
void r4300i_SPECIAL_SLLV (usf_state_t * state) {
	if (state->Opcode.rd == 0) { return; }
	state->GPR[state->Opcode.rd].DW = (state->GPR[state->Opcode.rt].W[0] << (state->GPR[state->Opcode.rs].UW[0] & 0x1F));
}

void r4300i_SPECIAL_SRLV (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = (int32_t)(state->GPR[state->Opcode.rt].UW[0] >> (state->GPR[state->Opcode.rs].UW[0] & 0x1F));
}
...
void r4300i_SPECIAL_JR (usf_state_t * state) {
	state->NextInstruction = DELAY_SLOT;
	state->JumpToLocation = state->GPR[state->Opcode.rs].UW[0];
}

void r4300i_SPECIAL_JALR (usf_state_t * state) {
	state->NextInstruction = DELAY_SLOT;
	state->JumpToLocation = state->GPR[state->Opcode.rs].UW[0];
	state->GPR[state->Opcode.rd].DW = (int32_t)(state->PROGRAM_COUNTER + 8);
}
...
void r4300i_SPECIAL_MFHI (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = state->HI.DW;
}

void r4300i_SPECIAL_MTHI (usf_state_t * state) {
	state->HI.DW = state->GPR[state->Opcode.rs].DW;
}

void r4300i_SPECIAL_MFLO (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = state->LO.DW;
}

void r4300i_SPECIAL_MTLO (usf_state_t * state) {
	state->LO.DW = state->GPR[state->Opcode.rs].DW;
}

void r4300i_SPECIAL_DSLLV (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = state->GPR[state->Opcode.rt].DW << (state->GPR[state->Opcode.rs].UW[0] & 0x3F);
}

void r4300i_SPECIAL_DSRLV (usf_state_t * state) {
	state->GPR[state->Opcode.rd].UDW = state->GPR[state->Opcode.rt].UDW >> (state->GPR[state->Opcode.rs].UW[0] & 0x3F);
}
...
void r4300i_SPECIAL_MULT (usf_state_t * state) {
	state->HI.DW = (int64_t)(state->GPR[state->Opcode.rs].W[0]) * (int64_t)(state->GPR[state->Opcode.rt].W[0]);
	state->LO.DW = state->HI.W[0];
	state->HI.DW = state->HI.W[1];
}

void r4300i_SPECIAL_MULTU (usf_state_t * state) {
	state->HI.DW = (uint64_t)(state->GPR[state->Opcode.rs].UW[0]) * (uint64_t)(state->GPR[state->Opcode.rt].UW[0]);
	state->LO.DW = state->HI.W[0];
	state->HI.DW = state->HI.W[1];
}

void r4300i_SPECIAL_DIV (usf_state_t * state) {
	if ( state->GPR[state->Opcode.rt].UDW != 0 ) {
		state->LO.DW = state->GPR[state->Opcode.rs].W[0] / state->GPR[state->Opcode.rt].W[0];
		state->HI.DW = state->GPR[state->Opcode.rs].W[0] % state->GPR[state->Opcode.rt].W[0];
	} else {
	}
}
...
void r4300i_SPECIAL_DIVU (usf_state_t * state) {
	if ( state->GPR[state->Opcode.rt].UDW != 0 ) {
		state->LO.DW = (int32_t)(state->GPR[state->Opcode.rs].UW[0] / state->GPR[state->Opcode.rt].UW[0]);
		state->HI.DW = (int32_t)(state->GPR[state->Opcode.rs].UW[0] % state->GPR[state->Opcode.rt].UW[0]);
	} else {
	}
}
...
void r4300i_SPECIAL_DMULT (usf_state_t * state) {
	...
	state->LO.UDW = (uint64_t)state->GPR[state->Opcode.rs].UW[0] * (uint64_t)state->GPR[state->Opcode.rt].UW[0];
	Tmp[0].UDW = (int64_t)state->GPR[state->Opcode.rs].W[1] * (int64_t)(uint64_t)state->GPR[state->Opcode.rt].UW[0];
	Tmp[1].UDW = (int64_t)(uint64_t)state->GPR[state->Opcode.rs].UW[0] * (int64_t)state->GPR[state->Opcode.rt].W[1];
	state->HI.UDW = (int64_t)state->GPR[state->Opcode.rs].W[1] * (int64_t)state->GPR[state->Opcode.rt].W[1];
...
void r4300i_SPECIAL_DMULTU (usf_state_t * state) {
	...
	state->LO.UDW = (uint64_t)state->GPR[state->Opcode.rs].UW[0] * (uint64_t)state->GPR[state->Opcode.rt].UW[0];
	Tmp[0].UDW = (uint64_t)state->GPR[state->Opcode.rs].UW[1] * (uint64_t)state->GPR[state->Opcode.rt].UW[0];
	Tmp[1].UDW = (uint64_t)state->GPR[state->Opcode.rs].UW[0] * (uint64_t)state->GPR[state->Opcode.rt].UW[1];
	state->HI.UDW = (uint64_t)state->GPR[state->Opcode.rs].UW[1] * (uint64_t)state->GPR[state->Opcode.rt].UW[1];
...
void r4300i_SPECIAL_DDIV (usf_state_t * state) {
	if ( state->GPR[state->Opcode.rt].UDW != 0 ) {
		state->LO.DW = state->GPR[state->Opcode.rs].DW / state->GPR[state->Opcode.rt].DW;
		state->HI.DW = state->GPR[state->Opcode.rs].DW % state->GPR[state->Opcode.rt].DW;
	} else {
	}
}
...
void r4300i_SPECIAL_DDIVU (usf_state_t * state) {
	if ( state->GPR[state->Opcode.rt].UDW != 0 ) {
		state->LO.UDW = state->GPR[state->Opcode.rs].UDW / state->GPR[state->Opcode.rt].UDW;
		state->HI.UDW = state->GPR[state->Opcode.rs].UDW % state->GPR[state->Opcode.rt].UDW;
	} else {
	}
}

void r4300i_SPECIAL_ADD (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = state->GPR[state->Opcode.rs].W[0] + state->GPR[state->Opcode.rt].W[0];
}
...
void r4300i_SPECIAL_AND (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = state->GPR[state->Opcode.rs].DW & state->GPR[state->Opcode.rt].DW;
}
...
void r4300i_SPECIAL_SLT (usf_state_t * state) {
	if (state->GPR[state->Opcode.rs].DW < state->GPR[state->Opcode.rt].DW) {
		state->GPR[state->Opcode.rd].DW = 1;
	} else {
		state->GPR[state->Opcode.rd].DW = 0;
	}
}

void r4300i_SPECIAL_SLTU (usf_state_t * state) {
	if (state->GPR[state->Opcode.rs].UDW < state->GPR[state->Opcode.rt].UDW) {
		state->GPR[state->Opcode.rd].DW = 1;
	} else {
		state->GPR[state->Opcode.rd].DW = 0;
	}
...
void r4300i_SPECIAL_DSLL (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = (state->GPR[state->Opcode.rt].DW << state->Opcode.sa);
}

void r4300i_SPECIAL_DSRL (usf_state_t * state) {
	state->GPR[state->Opcode.rd].UDW = (state->GPR[state->Opcode.rt].UDW >> state->Opcode.sa);
}
...
void r4300i_SPECIAL_DSLL32 (usf_state_t * state) {
	state->GPR[state->Opcode.rd].DW = (state->GPR[state->Opcode.rt].DW << (state->Opcode.sa + 32));
}

void r4300i_SPECIAL_DSRL32 (usf_state_t * state) {
   state->GPR[state->Opcode.rd].UDW = (state->GPR[state->Opcode.rt].UDW >> (state->Opcode.sa + 32));
}
...
void r4300i_COP0_MF (usf_state_t * state) {
	state->GPR[state->Opcode.rt].DW = (int32_t)state->CP0[state->Opcode.rd];
}

void r4300i_COP0_MT (usf_state_t * state) {
	switch (state->Opcode.rd) {
	case 0: //Index
	case 2: //EntryLo0
	case 3: //EntryLo1
	case 5: //PageMask
	...
	case 29: //Tag Hi
	case 30: //ErrEPC
		state->CP0[state->Opcode.rd] = state->GPR[state->Opcode.rt].UW[0];
		break;
	case 4: //Context
		state->CP0[state->Opcode.rd] = state->GPR[state->Opcode.rt].UW[0] & 0xFF800000;
		break;
	case 9: //Count
		state->CP0[state->Opcode.rd]= state->GPR[state->Opcode.rt].UW[0];
	...
	case 11: //Compare
		...
		break;
	case 12: //Status
		if ((state->CP0[state->Opcode.rd] ^ state->GPR[state->Opcode.rt].UW[0]) != 0) {
			state->CP0[state->Opcode.rd] = state->GPR[state->Opcode.rt].UW[0];
			SetFpuLocations(state);
		} else {
			state->CP0[state->Opcode.rd] = state->GPR[state->Opcode.rt].UW[0];
		}
		if ((state->CP0[state->Opcode.rd] & 0x18) != 0) {
		}
		CheckInterrupts(state);
		break;
	case 13: //cause
		state->CP0[state->Opcode.rd] &= 0xFFFFCFF;
		break;
	default:
...
void r4300i_COP1_MF (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	state->GPR[state->Opcode.rt].DW = *(int32_t *)state->FPRFloatLocation[state->Opcode.fs];
}

void r4300i_COP1_DMF (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	state->GPR[state->Opcode.rt].DW = *(int64_t *)state->FPRDoubleLocation[state->Opcode.fs];
}

void r4300i_COP1_CF (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	if (state->Opcode.fs != 31 && state->Opcode.fs != 0) {
		return;
	}
	state->GPR[state->Opcode.rt].DW = (int32_t)state->FPCR[state->Opcode.fs];
}

void r4300i_COP1_MT (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(int32_t *)state->FPRFloatLocation[state->Opcode.fs] = state->GPR[state->Opcode.rt].W[0];
}

void r4300i_COP1_DMT (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(int64_t *)state->FPRDoubleLocation[state->Opcode.fs] = state->GPR[state->Opcode.rt].DW;
}

void r4300i_COP1_CT (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	if (state->Opcode.fs == 31) {
		state->FPCR[state->Opcode.fs] = state->GPR[state->Opcode.rt].W[0];
		return;
	}
}
...
void r4300i_COP1_S_SQRT (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (float)sqrt(*(float *)state->FPRFloatLocation[state->Opcode.fs]);
}

void r4300i_COP1_S_ABS (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (float)fabs(*(float *)state->FPRFloatLocation[state->Opcode.fs]);
}

void r4300i_COP1_S_MOV (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = *(float *)state->FPRFloatLocation[state->Opcode.fs];
}

void r4300i_COP1_S_NEG (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (*(float *)state->FPRFloatLocation[state->Opcode.fs] * -1.0f);
}

void r4300i_COP1_S_TRUNC_L (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	//_controlfp(_RC_CHOP,_MCW_RC);
	Float_RoundToInteger64(&*(int64_t *)state->FPRDoubleLocation[state->Opcode.fd],&*(float *)state->FPRFloatLocation[state->Opcode.fs]);
}
...
void r4300i_COP1_S_ROUND_W (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	Float_RoundToInteger32(&*(int32_t *)state->FPRFloatLocation[state->Opcode.fd],&*(float *)state->FPRFloatLocation[state->Opcode.fs]);
}
...
void r4300i_COP1_S_CVT_D (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = (double)(*(float *)state->FPRFloatLocation[state->Opcode.fs]);
}
...
void r4300i_COP1_S_CMP (usf_state_t * state) {
	...
	Temp0 = *(float *)state->FPRFloatLocation[state->Opcode.fs];
	Temp1 = *(float *)state->FPRFloatLocation[state->Opcode.ft];
	...
	condition = ((state->Opcode.funct & 4) && less) | ((state->Opcode.funct & 2) && equal) |
		((state->Opcode.funct & 1) && unorded);
...
void r4300i_COP1_D_ADD (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = *(double *)state->FPRDoubleLocation[state->Opcode.fs] + *(double *)state->FPRDoubleLocation[state->Opcode.ft];
}
...
void r4300i_COP1_D_SQRT (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = (double)sqrt(*(double *)state->FPRDoubleLocation[state->Opcode.fs]);
}

void r4300i_COP1_D_ABS (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = fabs(*(double *)state->FPRDoubleLocation[state->Opcode.fs]);
}

void r4300i_COP1_D_MOV (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(int64_t *)state->FPRDoubleLocation[state->Opcode.fd] = *(int64_t *)state->FPRDoubleLocation[state->Opcode.fs];
}

void r4300i_COP1_D_NEG (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = (*(double *)state->FPRDoubleLocation[state->Opcode.fs] * -1.0);
}

void r4300i_COP1_D_TRUNC_L (usf_state_t * state) {	//added by Witten
	TEST_COP1_USABLE_EXCEPTION
	Double_RoundToInteger64(&*(int64_t *)state->FPRFloatLocation[state->Opcode.fd],&*(double *)state->FPRDoubleLocation[state->Opcode.fs] );
}
...
void r4300i_COP1_D_FLOOR_L (usf_state_t * state) {	//added by Witten
	TEST_COP1_USABLE_EXCEPTION
	Double_RoundToInteger64(&*(int64_t *)state->FPRDoubleLocation[state->Opcode.fd],&*(double *)state->FPRFloatLocation[state->Opcode.fs]);
}

void r4300i_COP1_D_ROUND_W (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	Double_RoundToInteger32(&*(int32_t *)state->FPRFloatLocation[state->Opcode.fd],&*(double *)state->FPRDoubleLocation[state->Opcode.fs] );
}
...
void r4300i_COP1_D_FLOOR_W (usf_state_t * state) {	//added by Witten
	TEST_COP1_USABLE_EXCEPTION
	Double_RoundToInteger32(&*(int32_t *)state->FPRDoubleLocation[state->Opcode.fd],&*(double *)state->FPRFloatLocation[state->Opcode.fs]);
}

void r4300i_COP1_D_CVT_S (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (float)*(double *)state->FPRDoubleLocation[state->Opcode.fs];
}
...
void r4300i_COP1_D_CVT_L (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	Double_RoundToInteger64(&*(int64_t *)state->FPRDoubleLocation[state->Opcode.fd],&*(double *)state->FPRDoubleLocation[state->Opcode.fs]);
}
...
void r4300i_COP1_D_CMP (usf_state_t * state) {
	...
	Temp0.DW = *(int64_t *)state->FPRDoubleLocation[state->Opcode.fs];
	Temp1.DW = *(int64_t *)state->FPRDoubleLocation[state->Opcode.ft];
...
void r4300i_COP1_W_CVT_S (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (float)*(int32_t *)state->FPRFloatLocation[state->Opcode.fs];
}

void r4300i_COP1_W_CVT_D (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = (double)*(int32_t *)state->FPRFloatLocation[state->Opcode.fs];
}

/************************** COP1: L functions ************************/
void r4300i_COP1_L_CVT_S (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(float *)state->FPRFloatLocation[state->Opcode.fd] = (float)*(int64_t *)state->FPRDoubleLocation[state->Opcode.fs];
}

void r4300i_COP1_L_CVT_D (usf_state_t * state) {
	TEST_COP1_USABLE_EXCEPTION
	*(double *)state->FPRDoubleLocation[state->Opcode.fd] = (double)*(int64_t *)state->FPRDoubleLocation[state->Opcode.fs];
}

/************************** Other functions **************************/
void R4300i_UnknownOpcode (usf_state_t * state) {
	DisplayError("Unkniown X86 Opcode.\tPC:%08x\tOp:%08x\n", state->PROGRAM_COUNTER,state->Opcode.Hex);
	StopEmulation(state);
}
