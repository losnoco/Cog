...
// 4 tap LFSR plus two tap adder
// What's the source? I made it the eff up!
static int16_t handleRng(JT1Player* SELF) {
	uint32_t rng = SELF->randomLFSR;
	uint32_t rngXor = (rng & 1) * 0x13050000;
	rng = ((rng >> 1) ^ rngXor) + 5;
	SELF->randomLFSR = rng;
	return (int16_t)rng;
}
	...
	for (f=0; f<SE_EFF_INST; f++) {
		...
		switch (fx->effecttype) {
			...
			case 3: { // Filter
					...
					sw = &vc->waves[256 * src];
					s = (int16_t) fx->effectspd;
					if(s > 12) s = 12; //not more than 12 times...it slows down too much
					for (t = 0; t < s; t++) {
						dw[0] = (sw[255] + sw[1]) >> 1;
						for (i = 1; i < 255; i++) {
			...
			case 5: { // Resonance
				...
				// init
					...
					if(fx->osceffect == 0) {
						...
						if(fx->oscflg) {
							centerFreq = (double)(fx->effectvar1 * 20);
							bandwidth = (double)(sw2[c] + 32768) / 16.0;
                        } else {
							centerFreq = (double)(sw2[c] + 32768) / 13.0;
							bandwidth = (double)(fx->effectvar2 * 16);
                        }
                    }

					vfx->b2 = exp(-(2.0 * M_PI) * (bandwidth / 22000.0));
					vfx->b1 = (-4.0 * vfx->b2) / (1.0 + vfx->b2) * cos(2.0 * M_PI * (centerFreq / 22000.0));
					vfx->a0 = (1.0 - vfx->b2) * sqrt(1.0 - (vfx->b1 * vfx->b1) / (4.0 * vfx->b2));

					for (i = 0; i < 256; i++) {
						double o = vfx->a0 * ((double)(sw1[i])/32768.0) - vfx->b1 * vfx->y1 - vfx->b2 * vfx->y2;

						vfx->y2 = vfx->y1;
						vfx->y1 = o;
						if(o > .9999) o = .9999;
						if(o < -.9999) o = -.9999;
						dw[i] = (int16_t)(o * 32768.0); // XXX original code could clip
					}
				}
				break;
			case 6: { // Reso Whistle
			...
			case 7: { // Morphing
				...
				// init
					...
					m2 = 255 - m1;
					for (i = 0; i < 256; i++) {
						int32_t a = (((int32_t)sw1[i] * m1) / 256) + (((int32_t)sw2[i] * m2) / 256);
						dw[i] = (int16_t)(a);
					}
				}
				break;
			case 8: { // Dyna-Morphing
				//  We assume here that in presets location: 1536 there is always a sine wave.
					int32_t dest, src1, src2, osc;
			...
			case 12: { // Clipper
				...
				// init
					...
					for (i = 0; i < 256; i++) {
						int32_t a = ((int32_t)sw1[i] * m1) / 16;
						if(a < -32767) a = -32767; // XXX I guess matches original code?
						if(a > 32767) a = 32767;
						dw[i] = (int16_t)(a);
					}
				}
			...
			case 13: { // bandpass
				...
				// init
					...
					double w0 = 228.0 + (double)freq;
					_2_pi_w0 = (int32_t)(2.0 * M_PI * w0);
					...
					for (i=0; i<256; i++) {
						...
						s = Vlp;
						if(s > 32767) s = 32767;
                        if(s < -32767) s = -32767; // XXX original code clipped like this
			...
			case 17: { // Squash
				...
				// init
					...
					for (i = 0; i < 256; i++) {
						int32_t a, b;
						b = sw1[m2 >> 8];
						a = sw1[((m2 >> 8) + 1) & 255]; // XXX added wrap
						a *= (m2 & 255);
						b *= (255 - (m2 & 255));
						a = (a >> 8) + (b >> 8);
						dw[i] = a;
...
static void handleInstrument(JT1Player* SELF, int32_t channr) {
	...
	//update freq
	int32_t k = SELF->song->arpTable[(ins->arpeggio * 16) + vc->arpcnt];
    vc->arpcnt = (vc->arpcnt + 1) & 15;
...
static void playInstrument(JT1Player* SELF, int32_t channr, int32_t instNum, int32_t note) {
	...
	vc = &SELF->voices[channr];
	if(vc->instrument == -1 && instNum == 0) return; // no instrument 0 on a muted channel... because there was no previous instrument
...
static void handleSong(JT1Player* SELF) {
	...
	int32_t absPosition = SELF->absPosition + SELF->timeSpd;
	...
	if (SELF->patternDelay == 0) {
		...
		if (SELF->playMode == SE_PM_PATTERN) {
			...
			// Mark the loop start for play position reporting
			if(SELF->playPosition == SELF->subsong->looppos && SELF->playStep == SELF->subsong->loopstep) {
				SELF->loopStart = SELF->absPosition;
			}
...
static void clearSoundBuffers(JT1Player* SELF) {
	...
	memset(SELF->leftDelayBuffer,  0, SE_DELAYLEN * sizeof(int16_t));
	memset(SELF->rightDelayBuffer, 0, SE_DELAYLEN * sizeof(int16_t));
...
JT1Player* jaytrax_init(void) {
	...
	if (!isStaticInit) {
		...
		for (j = 0; j < SE_NROFFINETUNESTEPS; j++) {
			for (i = 0; i < 128; i++) {
				// formula = root(x/12)
				f = ((i + 3) * 16) - j;   // We'll start with A because it's 440Hz
				f /= 192.0;
				f = pow(y, f);
				f *= 220.0;
		...
		//sine table
		for (i = 0; i < 256; i++) {
			sineTab[i] = (int16_t)(sin((M_PI * i) / 128.0) * 32760.0);
		}
	...
	SELF->playPosition = 0;  // Where is the song now?
	SELF->playStep = 0;
	SELF->subsongNr = 0;
	SELF->playFlg = 0;
	SELF->pauseFlg = 0;
...
void jaytrax_renderChunk(JT1Player* SELF, int16_t* outbuf, int32_t nrofsamples, float frequency) {
	int16_t ic, is;
	int32_t r;
	...
	uint32_t echodelaywrap;
	int16_t chanNr;
	// we calc nrofsamples samples in blocks of 'timeCnt' big (the songspd)
	...
	while (nrofsamples > 0) {
		...
		frameLen = (int32_t)floor((SELF->timeSpd * frequency) / 44100.0 + 0.5);
		...
		if (!outbuf) {
			...
			if (!SELF->song || !SELF->subsong || SELF->subsong->nrofchans == 0) {
				//clean renderbuffer
				memset(outbuf, 0, sizeof(int16_t) * nos);
				r += (nos * 2);
            } else {
				...
				for(ic = 0; ic < chanNr; ic++) {
					...
					if (instnr == -1) { // mute?
						...
						if(vc->sampledata) {
							...
							vc->waveLength = (inst->wavelength << 8) - 1;  //fixed point 8 bit (last 8 bits should be set)
						}
					}

					//calculate frequency
					if (vc->curfreq < 10) vc->curfreq = 10;
					vc->freqOffset = (int32_t) floor((256.0 * vc->curfreq) / frequency * (vc->freqOffset < 0 ? -1.0 : 1.0) + 0.5);
				...
				echodelaywrap = (uint32_t)floor((double)echodelaytime / (44100.0 / frequency) + 0.5);
				if(echodelaywrap > SE_DELAYLEN) echodelaywrap = SE_DELAYLEN;
...
int32_t jaytrax_getLength(JT1Player* SELF, int subsongnr, int loopCnt, float frequency) {
	int32_t length = 0;
	int32_t lengthMax = floor(frequency * 60.0 * 30.0 + 0.5);
	int32_t frameLen;
	jaytrax_changeSubsong(SELF, subsongnr);
	while (SELF->playFlg && SELF->loopCnt < loopCnt && length < lengthMax) {
		frameLen = floor(((double)SELF->timeSpd * frequency) / 44100.0 + 0.5);
		advanceSong(SELF);
		length += frameLen;
	}
