...
u32 FASTCALL C68k_Read_Dummy(void *param, const u32 adr);
void FASTCALL C68k_Write_Dummy(void *param, const u32 adr, u32 data);
...
s32  FASTCALL C68k_Interrupt_Ack_Dummy(void *param, s32 level);
void FASTCALL C68k_Reset_Dummy(void *param);
...
u32 FASTCALL C68k_Read_Dummy(UNUSED void *param, UNUSED const u32 adr)
{
    return 0;
}

void FASTCALL C68k_Write_Dummy(UNUSED void *param, UNUSED const u32 adr, UNUSED u32 data)
{

}

s32 FASTCALL C68k_Interrupt_Ack_Dummy(UNUSED void *param, s32 level)
{
    // return vector
    return (C68K_INTERRUPT_AUTOVECTOR_EX + level);
}

void FASTCALL C68k_Reset_Dummy(UNUSED void *param)
{
...
u32 C68k_Read_Byte(c68k_struc *cpu, u32 adr)
{
    return cpu->Read_Byte(cpu->Callback_Param, adr);
}

u32 C68k_Read_Word(c68k_struc *cpu, u32 adr)
{
    return cpu->Read_Word(cpu->Callback_Param, adr);
}

u32 C68k_Read_Long(c68k_struc *cpu, u32 adr)
{
#ifdef C68K_BIG_ENDIAN
    return (cpu->Read_Word(cpu->Callback_Param, adr) << 16) | (cpu->Read_Word(cpu->Callback_Param, adr + 2) & 0xFFFF);
#else
    return (cpu->Read_Word(cpu->Callback_Param, adr) << 16) | (cpu->Read_Word(cpu->Callback_Param, adr + 2) & 0xFFFF);
#endif
}
...
void C68k_Write_Byte(c68k_struc *cpu, u32 adr, u32 data)
{
    cpu->Write_Byte(cpu->Callback_Param, adr, data);
}

void C68k_Write_Word(c68k_struc *cpu, u32 adr, u32 data)
{
    cpu->Write_Word(cpu->Callback_Param, adr, data);
}

void C68k_Write_Long(c68k_struc *cpu, u32 adr, u32 data)
{
#ifdef C68K_BIG_ENDIAN
    cpu->Write_Word(cpu->Callback_Param, adr, data >> 16);
    cpu->Write_Word(cpu->Callback_Param, adr + 2, data & 0xFFFF);
#else
    cpu->Write_Word(cpu->Callback_Param, adr, data >> 16);
    cpu->Write_Word(cpu->Callback_Param, adr + 2, data & 0xFFFF);
#endif
...
void C68k_Set_Callback_Param(c68k_struc *cpu, void *Param)
{
    cpu->Callback_Param = Param;
}
