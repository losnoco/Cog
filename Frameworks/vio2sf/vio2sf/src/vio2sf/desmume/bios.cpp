...
TEMPLATE static uint32_t bios_nop(armcpu_t *)
{
	return 3;
}

TEMPLATE static uint32_t WaitByLoop(armcpu_t *cpu)
{
	uint32_t elapsed;

	if (PROCNUM == ARMCPU_ARM9)
	{
		if (cpu->st->cp15.ctrl & ((1 << 16) | (1 << 18))) // DTCM or ITCM is on (cache)
			elapsed = cpu->R[0] * 2;
		else
			elapsed = cpu->R[0] * 8;
	}
...
TEMPLATE static uint32_t wait4IRQ(armcpu_t *cpu)
{
	cpu->waitIRQ = true;
...
TEMPLATE static uint32_t intrWaitARM(armcpu_t *cpu)
{
	// TODO - account for differences between arm7 and arm9 (according to gbatek, the "bug doesn't work")

	const uint32_t intrFlagAdr = PROCNUM == ARMCPU_ARM7 ? 0x380FFF8 : (cpu->st->cp15.DTCMRegion & 0xFFFFF000) + 0x3FF8;
	...
	_MMU_write32<PROCNUM>(cpu->st, 0x04000208, 1);

	// analyze the OS irq flag variable
	uint32_t intr = _MMU_read32<PROCNUM>(cpu->st, intrFlagAdr);
	...
	{
		intr ^= intrFlag;
		_MMU_write32<PROCNUM>(cpu->st, intrFlagAdr, intr);
...
TEMPLATE static uint32_t waitVBlankARM(armcpu_t *cpu)
{
	cpu->R[0] = 1;
	cpu->R[1] = 1;
...
TEMPLATE static uint32_t sleep(armcpu_t *cpu)
{
	_MMU_write08<PROCNUM>(cpu->st, 0x04000301, 0xC0);
	return 1;
}

TEMPLATE static uint32_t divide(armcpu_t *cpu)
{
	int32_t num = cpu->R[0];
...
TEMPLATE static uint32_t copy(armcpu_t *cpu)
{
		...
		case 0:
			...
			switch (BIT24(cnt))
			{
				case 0:
					...
					while (cnt)
					{
						_MMU_write16<PROCNUM>(cpu->st, dst, _MMU_read16<PROCNUM>(cpu->st, src));
						--cnt;
						dst += 2;
						src += 2;
					}
				...
				case 1:
				{
					uint16_t val = _MMU_read16<PROCNUM>(cpu->st, src);
					cnt &= 0x1FFFFF;
					while (cnt)
					{
						_MMU_write16<PROCNUM>(cpu->st, dst, val);
						--cnt;
						dst += 2;
					}
					break;
		...
		case 1:
			...
			switch (BIT24(cnt))
			{
				case 0:
					...
					while (cnt)
					{
						_MMU_write32<PROCNUM>(cpu->st, dst, _MMU_read32<PROCNUM>(cpu->st, src));
						--cnt;
						dst += 4;
						src += 4;
					}
				...
				case 1:
				{
					uint32_t val = _MMU_read32<PROCNUM>(cpu->st, src);
					cnt &= 0x1FFFFF;
					while (cnt)
					{
						_MMU_write32<PROCNUM>(cpu->st, dst, val);
						--cnt;
						dst += 4;
					}
					break;
...
TEMPLATE static uint32_t fastCopy(armcpu_t *cpu)
{
...
TEMPLATE static uint32_t LZ77UnCompVram(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = _MMU_read32<PROCNUM>(cpu->st, source);
	source += 4;
	...
	while (len > 0)
	{
		uint8_t d = _MMU_read08<PROCNUM>(cpu->st, source++);
		...
		if (d)
		{
			for (i1 = 0; i1 < 8; ++i1)
			{
				if (d & 0x80)
				{
					uint16_t data = _MMU_read08<PROCNUM>(cpu->st, source++) << 8;
					data |= _MMU_read08<PROCNUM>(cpu->st, source++);
					int length = (data >> 12) + 3;
					int offset = data & 0x0FFF;
					uint32_t windowOffset = dest + byteCount - offset - 1;
					for (i2 = 0; i2 < length; ++i2)
					{
						writeValue |= _MMU_read08<PROCNUM>(cpu->st, windowOffset++) << byteShift;
						byteShift += 8;
						++byteCount;

						if (byteCount == 2)
						{
							_MMU_write16<PROCNUM>(cpu->st, dest, writeValue & 0xFFFF);
							dest += 2;
							byteCount = 0;
							byteShift = 0;
							writeValue = 0;
				...
				{
					writeValue |= _MMU_read08<PROCNUM>(cpu->st, source++) << byteShift;
					byteShift += 8;
					++byteCount;
					if (byteCount == 2)
					{
...
TEMPLATE static uint32_t LZ77UnCompWram(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = _MMU_read32<PROCNUM>(cpu->st, source);
		...
		if (d)
		{
			for (i1 = 0; i1 < 8; ++i1)
			{
				if (d & 0x80)
				{
					...
					for (i2 = 0; i2 < length; ++i2)
					{
						_MMU_write08<PROCNUM>(cpu->st, dest++, _MMU_read08<PROCNUM>(cpu->st, windowOffset++));
						--len;
						if (!len)
							return 0;
					}
				...
				{
					_MMU_write08<PROCNUM>(cpu->st, dest++, _MMU_read08<PROCNUM>(cpu->st, source++));
					--len;
					if (!len)
						return 0;
				}
...
TEMPLATE static uint32_t RLUnCompVram(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = _MMU_read32<PROCNUM>(cpu->st, source);
	...
	while (len > 0)
	{
		...
		if (d & 0x80)
		{
			uint8_t data = _MMU_read08<PROCNUM>(cpu->st, source++);
			l += 3;
			for (i = 0; i < l; ++i)
			{
...
TEMPLATE static uint32_t RLUnCompWram(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = _MMU_read32<PROCNUM>(cpu->st, source);
	...
	while (len > 0)
	{
		...
		if (d & 0x80)
		{
			...
			for (i = 0; i < l; ++i)
			{
				_MMU_write08<PROCNUM>(cpu->st, dest++, data);
				--len;
				if (!len)
					return 0;
			}
...
TEMPLATE static uint32_t UnCompHuffman(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = _MMU_read32<PROCNUM>(cpu->st, source);
	...
	uint8_t treeSize = _MMU_read08<PROCNUM>(cpu->st, source++);
	...
	uint32_t data = _MMU_read32<PROCNUM>(cpu->st, source);
	source += 4;

	int pos = 0;
	uint8_t rootNode = _MMU_read08<PROCNUM>(cpu->st, treeStart);
	...
	if ((header & 0x0F) == 8)
	{
		while (len > 0)
		{
			...
			if (data & mask)
			{
				...
				if (currentNode & 0x40)
					writeData = 1;
				currentNode = _MMU_read08<PROCNUM>(cpu->st, treeStart + pos + 1);
			}
			else
			{
				// left
				if (currentNode & 0x80)
					writeData = 1;
				currentNode = _MMU_read08<PROCNUM>(cpu->st, treeStart + pos);
			}
			...
			if (writeData)
			{
				...
				if (byteCount == 4)
				{
					byteCount = 0;
					byteShift = 0;
					_MMU_write32<PROCNUM>(cpu->st, dest, writeValue);
					writeValue = 0;
					dest += 4;
					len -= 4;
				}
			...
			{
				mask = 0x80000000;
				data = _MMU_read32<PROCNUM>(cpu->st, source);
				source += 4;
			}
		}
	}
...
TEMPLATE static uint32_t BitUnPack(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	uint32_t header = cpu->R[2];

	int len = _MMU_read16<PROCNUM>(cpu->st, header);
	int bits = _MMU_read08<PROCNUM>(cpu->st, header + 2);
	switch (bits)
	{
		case 1:
		case 2:
	...
	int dataSize = _MMU_read08<PROCNUM>(cpu->st, header + 3);
	switch (dataSize)
	{
		case 1:
		case 2:
	...
	int revbits = 8 - bits;
	uint32_t base = _MMU_read32<PROCNUM>(cpu->st, header + 4);
	...
	while (1)
	{
		...
		uint8_t b = _MMU_read08<PROCNUM>(cpu->st, source);
		++source;
		int bitcount = 0;
		while (1)
		{
      ...
      if (temp || addBase) {
        temp += base;
			...
			if (bitwritecount >= 32)
			{
				_MMU_write32<PROCNUM>(cpu->st, dest, data);
				dest += 4;
				data = 0;
				bitwritecount = 0;
			}
...
TEMPLATE static uint32_t Diff8bitUnFilterWram(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	CompressionHeader header(_MMU_read32<PROCNUM>(cpu->st, source));
	source += 4;
  ...
  if (PROCNUM == ARMCPU_ARM7) {
	...
	while (len > 0)
	{
		uint8_t diff = _MMU_read08<PROCNUM>(cpu->st, source++);
		data += diff;
		_MMU_write08<PROCNUM>(cpu->st, dest++, data);
		--len;
	}
...
TEMPLATE static uint32_t Diff16bitUnFilter(armcpu_t *cpu)
{
	uint32_t source = cpu->R[0];
	uint32_t dest = cpu->R[1];
	CompressionHeader header(_MMU_read32<PROCNUM>(cpu->st, source));
	...
	uint16_t data = _MMU_read16<PROCNUM>(cpu->st, source);
	source += 2;
	_MMU_write16<PROCNUM>(cpu->st, dest, data);
	dest += 2;
	len -= 2;

	while (len >= 2)
	{
		uint16_t diff = _MMU_read16<PROCNUM>(cpu->st, source);
		source += 2;
		data += diff;
		_MMU_write16<PROCNUM>(cpu->st, dest, data);
		dest += 2;
...
TEMPLATE static uint32_t bios_sqrt(armcpu_t *cpu)
{
	cpu->R[0] = static_cast<uint32_t>(std::sqrt(static_cast<double>(cpu->R[0])));
	return 1;
}

TEMPLATE static uint32_t CustomPost(armcpu_t *cpu)
{
  _MMU_write08<PROCNUM>(cpu->st, REG_POSTFLG, cpu->R[0]);
  return 1;
}

TEMPLATE static uint32_t CustomHalt(armcpu_t *cpu)
{
	_MMU_write08<PROCNUM>(cpu->st, REG_HALTCNT, cpu->R[2]);
	return 1;
}

TEMPLATE static uint32_t getSineTab(armcpu_t *cpu)
{
...
TEMPLATE static uint32_t getPitchTab(armcpu_t *cpu)
{
	// ds returns garbage according to gbatek, but we must protect ourselves
...
TEMPLATE static uint32_t getVolumeTab(armcpu_t *cpu)
{
	// ds returns garbage according to gbatek, but we must protect ourselves
...
TEMPLATE static uint32_t getCRC16(armcpu_t *cpu)
{
	// gbatek is wrong.
	...
	{
		currVal = _MMU_read16<PROCNUM>(cpu->st, datap + i * 2);
...
TEMPLATE static uint32_t isDebugger(armcpu_t *cpu)
{
...
TEMPLATE static uint32_t SoundBias(armcpu_t *cpu)
{
	uint32_t curBias = _MMU_read32<ARMCPU_ARM7>(cpu->st, 0x04000504);
	uint32_t newBias = !curBias ? 0x000:0x200;
	uint32_t delay = newBias > curBias ? newBias - curBias : curBias - newBias;

	_MMU_write32<ARMCPU_ARM7>(cpu->st, 0x04000504, newBias);
	return cpu->R[1] * delay;
}

TEMPLATE static uint32_t getBootProcs(armcpu_t *cpu)
{
	cpu->R[0] = 0x00000A2E;
	cpu->R[1] = 0x00002C3C;
	cpu->R[3] = 0x000005FF;
...
uint32_t (*ARM_swi_tab[][32])(armcpu_t *) =
{
	{
