...
#include <vio2sf/ARMJIT_Internal.h>
...
#endif
    if (MemoryFile != -1 && ftruncate(MemoryFile, MemoryTotalSize) < 0)
    {
        Log(LogLevel::Error, "Failed to allocate memory using ftruncate! (%s)", strerror(errno));
    }

    mmap(MemoryBase, MemoryTotalSize, PROT_READ | PROT_WRITE, ((MemoryFile != -1) ? MAP_SHARED : MAP_PRIVATE) | MAP_FIXED | MAP_ANONYMOUS, MemoryFile, 0);
#endif
