...
static inline void AddCycles(struct S9xState *st, int32_t n) { st->CPU.PrevCycles = st->CPU.Cycles; st->CPU.Cycles += n; S9xCheckInterrupts(st); while (st->CPU.Cycles >= st->CPU.NextEvent) S9xDoHEventProcessing(st); }
