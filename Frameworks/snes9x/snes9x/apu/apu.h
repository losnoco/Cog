...
#include <snes9x/resamplerSNSF.h>
...
struct S9xSPC
{
	...
	std::unique_ptr<uint8_t[]> landing_buffer;
	std::unique_ptr<uint8_t[]> shrink_buffer;

	std::unique_ptr<ResamplerSNSF> resampler;
...
void S9xAPUAllowTimeOverflow(struct S9xState *st, bool);

template<class ResamplerClass> bool S9xInitSound(struct S9xState *st, int, int);
