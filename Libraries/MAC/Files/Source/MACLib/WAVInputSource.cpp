...
{ 
    ...
    if (strcasecmp(pExtension, ".wav") == 0)
    {
...
int CWAVInputSource::AnalyzeSource()
{
	...
	RIFFHeader.nBytes = swap_endian32(RIFFHeader.nBytes);		
	...
	RIFFChunkHeader.nChunkBytes = swap_endian32(RIFFChunkHeader.nChunkBytes);
	...
	WAVFormatHeader.nFormatTag = swap_endian16(WAVFormatHeader.nFormatTag);
	WAVFormatHeader.nChannels = swap_endian16(WAVFormatHeader.nChannels);
	WAVFormatHeader.nSamplesPerSecond = swap_endian32(WAVFormatHeader.nSamplesPerSecond);
	WAVFormatHeader.nBytesPerSecond = swap_endian32(WAVFormatHeader.nBytesPerSecond);
	WAVFormatHeader.nBlockAlign = swap_endian16(WAVFormatHeader.nBlockAlign);
	WAVFormatHeader.nBitsPerSample = swap_endian16(WAVFormatHeader.nBitsPerSample);
