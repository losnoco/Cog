...
int CAPETag::Analyze()
{
    ...
    if (m_bHasID3Tag == FALSE)
    {
		...
		APETagFooter.m_nVersion = swap_endian32(APETagFooter.m_nVersion);
		APETagFooter.m_nSize = swap_endian32(APETagFooter.m_nSize);
		APETagFooter.m_nFields = swap_endian32(APETagFooter.m_nFields);
		APETagFooter.m_nFlags = swap_endian32(APETagFooter.m_nFlags);
...
int CAPETag::CreateID3Tag(ID3_TAG * pID3Tag)
{
    ...
    pID3Tag->Track = (unsigned char) atoi(GetANSIFromUTF16(cBuffer));
...
int CAPETag::LoadField(const char * pBuffer, int nMaximumBytes, int * pBytes)
{
	...
	nFieldValueSize = swap_endian32(nFieldValueSize);
    nLocation += 4;
	int nFieldFlags = *((int *) &pBuffer[nLocation]);
	nFieldFlags = swap_endian32(nFieldFlags);
    nLocation += 4;
