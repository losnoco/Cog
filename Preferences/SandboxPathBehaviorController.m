	...
	if(results && [results count] > 0) {
		for(SandboxToken *token in results) {
			...
			if(!isDisconnected) {
				...
				[self addObject:@{ @"path": token.path, @"valid": ((!bookmarkUrl || isStale) ? NSLocalizedString(@"ValidNo", @"Negative answer to whether a path is valid.") : NSLocalizedString(@"ValidYes", @"Positive answer to whether a path is valid.")), @"isFolder": @(token.folder), @"token": token }];
			} else {
				[self addObject:@{ @"path": token.path, @"valid": NSLocalizedString(@"ValidNo", @"Negative answer to whether a path is valid."), @"isFolder": @(token.folder), @"token": token }];
			}
		}
	}
}
		...
		if(token) {
			...
			if(err) {
				...
				[self addObject:@{@"path": [url path],
					              @"valid": NSLocalizedString(@"ValidYes", @"Positive answer to whether a path is valid."),
...
- (void)removeStaleEntries {
	...
	for(NSDictionary *entry in [[self arrangedObjects] copy]) {
		if([[entry objectForKey:@"valid"] isEqualToString:NSLocalizedString(@"ValidNo", @"Negative answer to whether a path is valid.")]) {
...
- (BOOL)matchesPath:(NSURL *)url {
	...
	for(NSDictionary *entry in [self arrangedObjects]) {
		if([[entry objectForKey:@"valid"] isEqualToString:NSLocalizedString(@"ValidYes", @"Positive answer to whether a path is valid.")]) {
